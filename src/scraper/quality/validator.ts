import type { ProductRecord, QualityIssue, QualityResult } from '../../shared/types'

export function evaluateProductQuality(product: ProductRecord): QualityResult {
  ...
  if (!product.description.trim()) info('NO_DESCRIPTION', 'No separate product description was found.')
  if (!product.assets.some((a) => a.type === 'image')) info('NO_IMAGE', 'No product image link was detected.')
  if (!product.assets.some((a) => a.type === 'document')) info('NO_DOCUMENT', 'No document link was detected.')
  if (!product.assets.some((a) => a.type === 'cad')) info('NO_CAD', 'No CAD link was detected.')
